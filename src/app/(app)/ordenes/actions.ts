...
export async function actualizarOrden(formData: FormData) {
  ...
  if (!numeroOrden || !clienteNombre) {
    redirect(`/ordenes/${ordenId}?error=Falta+número+de+orden+o+cliente`);
  }
  ...
  if (!cliente) {
    const { data: nuevo, error: errCliente } = await supabase
      ...
      .select("id")
      .single();
    if (errCliente) redirect(`/ordenes/${ordenId}?error=${encodeURIComponent(errCliente.message)}`);
    cliente = nuevo;
  }
...
export async function actualizarSuborden(formData: FormData) {
  const supabase = supabaseAdmin();
  const ordenId = String(formData.get("orden_id"));
  const subordenId = String(formData.get("suborden_id"));
  const numeroOrden = String(formData.get("numero_orden"));
  const numeroSuborden = Number(formData.get("numero_suborden"));
  const producto = String(formData.get("producto") || "");
  const pieza = String(formData.get("pieza") || "");
  ...
  // Reemplazamos los acabados seleccionados (borrar e insertar de nuevo
  // es más simple y seguro que calcular el diff).
  await supabase.from("subordenes_acabados").delete().eq("suborden_id", subordenId);
  if (tiposAcabadoIds.length) {
    const filas = tiposAcabadoIds.map((tipoAcabadoId) => ({
